...
const BudgetSummary = ({
  ...
  const remainingIsNegative = summary.remaining < 0;
  const remainingIsZero = summary.remaining === 0;
  const remainingLabel = remainingIsNegative ? 'חריגה' : 'נותר';
  const remainingNote = remainingIsNegative
    ? 'ההוצאה בפועל גבוהה מהתקציב'
    : remainingIsZero
      ? 'התקציב נוצל במלואו'
      : 'תקציב פחות ביצוע';
    ...
    <GlassCard className="budget-overview" padding="20px">
      ...
      <div className="budget-summary-grid" aria-label={`סיכום תקציב ${formatBudgetMonth(selectedMonth)}`}>
        <SummaryMetric
          ...
          note="סכום כל הקטגוריות המתוקצבות"
          loading={loading}
        ...
        <SummaryMetric
          ...
          note="מחושב מתנועות ההוצאה של החודש"
          tone="actual"
          loading={loading}
          unavailable={unavailable}
        />
        <SummaryMetric
          ...
          value={remainingIsNegative ? Math.abs(summary.remaining) : summary.remaining}
          note={remainingNote}
          tone={remainingIsNegative ? 'negative' : remainingIsZero ? 'neutral' : 'positive'}
          loading={loading}
          unavailable={unavailable}
        />
      </div>

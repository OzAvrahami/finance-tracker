-- Migration 010: explicit early-payoff and provider-adjustment accounting.
-- Cash remains in transactions; principal effects remain authoritative here.
...
ALTER TABLE public.loan_payments
  ...
  ADD COLUMN IF NOT EXISTS other_amount NUMERIC(24, 10) NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS balance_adjustment_amount NUMERIC(24, 10) NOT NULL DEFAULT 0;

ALTER TABLE public.loan_payments
  ALTER COLUMN installment_number DROP NOT NULL;

ALTER TABLE public.loan_payments
  DROP CONSTRAINT IF EXISTS loan_payments_loan_id_installment_number_key,
  DROP CONSTRAINT IF EXISTS loan_payments_installment_number_check,
  DROP CONSTRAINT IF EXISTS loan_payments_check,
...
ALTER TABLE public.loan_payments
  ...
  ADD CONSTRAINT loan_payments_cash_reconciliation_check
    CHECK (
      abs(payment_amount - principal_amount - interest_amount - other_amount)
        <= 0.00000001
    );

CREATE UNIQUE INDEX IF NOT EXISTS loan_payments_unique_installment
  ON public.loan_payments (loan_id, installment_number)
  WHERE payment_kind = 'installment';

CREATE UNIQUE INDEX IF NOT EXISTS loan_payments_unique_early_payoff
...
-- Keep normal transaction-driven accounting strictly installment-shaped.
...
-- Migration 009's bounded automatic RPC, adjusted to count regular
-- installments only and to write explicit zero adjustment fields.
...
-- Reassert Migration 008/009's PostgREST boundary after replacement.

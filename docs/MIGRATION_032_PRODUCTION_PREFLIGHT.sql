-- OWNER RUN ONLY. Read-only; one consolidated JSON result. Requires Migration 031.
-- Pause application/import/scheduler writes across preflight -> 032 -> postflight.
...
,
(SELECT md5(prosrc)='3a89a0b93d4a6260d8979562dc02c127' FROM pg_proc WHERE oid='public.savings_post_event_locked(uuid,jsonb)'::regprocedure) AS expected_event_engine,
(SELECT md5(prosrc)='9cf2a4b788b6db863f010201d768cadd' FROM pg_proc WHERE oid='public.get_savings_account(bigint,date,date,bigint,integer)'::regprocedure) AS expected_history_reader
), evidence AS (SELECT jsonb_build_object(
 'unchanged_functions_fingerprint',(SELECT md5(string_agg(pg_get_functiondef(oid)||coalesce(proacl::text,''),E'\n' ORDER BY oid::regprocedure::text)) FROM pg_proc WHERE pronamespace='public'::regnamespace AND prokind='f' AND proname NOT IN ('savings_post_event_locked','get_savings_account')),
...
SELECT jsonb_build_object('result',CASE WHEN NOT EXISTS(SELECT 1 FROM jsonb_each(to_jsonb(c)) WHERE value<>'true'::jsonb) THEN 'MIGRATION_032_PREFLIGHT_PASS' ELSE 'MIGRATION_032_PREFLIGHT_FAIL' END,'checks',to_jsonb(c),'evidence',e.body) FROM checks c CROSS JOIN evidence e;

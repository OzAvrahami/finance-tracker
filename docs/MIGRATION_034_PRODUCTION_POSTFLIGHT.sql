...
-- Pause application/import/scheduler writes across preflight -> 034 -> postflight.
...
,
 ...
 'unchanged_functions_fingerprint',(SELECT md5(string_agg(pg_get_functiondef(oid)||coalesce(proacl::text,''),E'\n' ORDER BY oid::regprocedure::text)) FROM pg_proc WHERE pronamespace='public'::regnamespace AND prokind='f' AND proname NOT IN ('savings_validate_account','update_savings_account','savings_post_event_locked','savings_assert_links','get_savings_account')),
...
SELECT jsonb_build_object('result',CASE WHEN NOT EXISTS(SELECT 1 FROM jsonb_each(to_jsonb(c)) WHERE value<>'true'::jsonb) THEN 'MIGRATION_034_POSTFLIGHT_PASS' ELSE 'MIGRATION_034_POSTFLIGHT_FAIL' END,'checks',to_jsonb(c),'evidence',e.body) FROM checks c CROSS JOIN evidence e;

...
SELECT jsonb_build_object('result',CASE WHEN NOT EXISTS(SELECT 1 FROM jsonb_each(to_jsonb(c)) WHERE value<>'true'::jsonb) THEN 'MIGRATION_034_PREFLIGHT_PASS' ELSE 'MIGRATION_034_PREFLIGHT_FAIL' END,'checks',to_jsonb(c),'evidence',e.body) FROM checks c CROSS JOIN evidence e;

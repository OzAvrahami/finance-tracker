-- OWNER RUN ONLY. Read-only; one consolidated JSON result. Requires Migration 032.
...
, to_regprocedure('public.apply_savings_surplus(uuid,text,jsonb)') IS NULL AS not_already_installed
...
SELECT jsonb_build_object('result',CASE WHEN NOT EXISTS(SELECT 1 FROM jsonb_each(to_jsonb(c)) WHERE value<>'true'::jsonb) THEN 'MIGRATION_033_PREFLIGHT_PASS' ELSE 'MIGRATION_033_PREFLIGHT_FAIL' END,'checks',to_jsonb(c),'evidence',e.body) FROM checks c CROSS JOIN evidence e;

...
,
(SELECT md5(prosrc)='50649162a3fab4275ebe1547c0dccde8' FROM pg_proc WHERE oid='public.savings_post_event_locked(uuid,jsonb)'::regprocedure) AS expected_event_engine,
...
SELECT jsonb_build_object('result',CASE WHEN NOT EXISTS(SELECT 1 FROM jsonb_each(to_jsonb(c)) WHERE value<>'true'::jsonb) THEN 'MIGRATION_032_POSTFLIGHT_PASS' ELSE 'MIGRATION_032_POSTFLIGHT_FAIL' END,'checks',to_jsonb(c),'evidence',e.body) FROM checks c CROSS JOIN evidence e;

...
- Repository migration history currently reaches Migration 025. Migration 024 is deployed and makes the consolidated Budget operation/item model authoritative; Migration 025 is the pending read/compatibility cleanup.
- Production object/data preflights independently verified the funded-budget foundation through Migration 024. No Finance Tracker applied-migration ledger exists, so these findings are object-state evidence rather than an authoritative execution history.
...
| Monthly budgets | Consolidated model deployed; cleanup implemented locally | Migration 024 provides eleven physical tables and one universal operation/item provenance system. Migration 025 removes temporary internal compatibility reads without changing product behavior. The combined recurring/month edit and full redesign remain future work. |
...
- Migration 024 is deployed. Migration 025 requires its exact consolidated boundary, performs no financial data transformation, and removes only obsolete internal views/helpers after replacing their dependencies.

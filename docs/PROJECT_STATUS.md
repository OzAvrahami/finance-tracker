...
- Repository migration history currently reaches Migration 020. Migration 020 is implemented locally and has not been applied to production.
...
| Monthly budgets | Month-override extension implemented locally, production migration pending | Funded month/read model, immutable opening snapshots, recurring initialization, balanced carryover, and month-specific base overrides with separate provenance. Savings disposition, rich reallocation/deficit UX, and the full summary redesign remain future work. |
...
- Migration 020 has not been applied to production. Its server/client paths require deployed and verified Migrations 017–019 before deployment.
...
- Unused-budget disposition/savings, category reallocation UX, explicit deficit-resolution UX, and the full funded-budget presentation remain deferred.

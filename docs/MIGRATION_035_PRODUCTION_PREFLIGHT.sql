...
,
to_regprocedure('public.get_savings_report(date,date,bigint)') IS NULL AS reporting_not_installed
...
SELECT jsonb_build_object('result',CASE WHEN NOT EXISTS(SELECT 1 FROM jsonb_each(to_jsonb(c)) WHERE value<>'true'::jsonb) THEN 'MIGRATION_035_PREFLIGHT_PASS' ELSE 'MIGRATION_035_PREFLIGHT_FAIL' END,'checks',to_jsonb(c),'evidence',e.body) FROM checks c CROSS JOIN evidence e;

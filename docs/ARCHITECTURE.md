...
Savings foundation is implemented locally under [SAV-02 / #38](https://github.com/OzAvrahami/finance-tracker/issues/38), using the approved [canonical specification](SAVINGS_V1_3_0_SPEC.md). [The implementation and reader audit](SAVINGS_FOUNDATION.md) records Migration 030: exactly two new tables, one summary view, three account RPCs, nine foundation helpers, eleven trigger attachments, six structurally changed existing tables and three replaced Budget views. Twenty-seven existing functions receive reader/structural compatibility changes. The initial design inventory of ten Savings RPCs, two Budget overloads and eleven helpers was delivered across #38–#42. SAV-07 adds one justified read-only report RPC and extends the existing transaction readers; each handoff enumerates its concrete replacements. The existing Budget 11-table/9-view boundary and Loan columns, grants, calculations and existing functions/triggers are preserved.

Cash and held Savings remain separate authorities. The foundation implements original-post prefix ordering, four protected void fields, the Loan-side link guard, immutable history and atomic opening/reserve retirement. Only the read-only budget_actual_transactions helper has the explicit service_role EXECUTE exception; other helpers are protected. Express provides authenticated account/configuration/read routes and the client provides the Loans-based management screens. SQL/Node reader filtering and cancelled-history handling are delivered before #39's cancellation commands and dispatch/refetch; #43 is an extension, not a safety prerequisite. SAV-02 was accepted by the owner on 2026-09-13. SAV-03 now implements manual cash through four RPCs and one private engine in Migration 031, two extended transaction-reader signatures and the existing transaction guard. No tables/views/columns/triggers are added by 031. Both UI entry points use the existing transaction form and one server/SQL accounting boundary; full application navigation is verified. Live filtering, cancelled detail, explicit detached void/reuse and basic cross-page refetch are delivered before #43. SAV-03 is completed after owner appearance confirmation and its documented verification. SAV-04 extends the same engine with actual capitalized/payout interest, four-case destination correction and noncash cancellation; Migration 032 replaces only savings_post_event_locked and get_savings_account. The latter reports reinstatement eligibility across complete replacement history. The Savings dialog and explicit interest-payout transaction category use the same audited boundary; ordinary form initialization/layout remains unchanged. Earnings never add cash to held balance. SAV-05 now implements named funded transfers in Migration 033: three surplus RPCs, two explicit Budget overloads, one private helper and fifteen existing function replacements plus the existing composition view. No relations are added. Budget funding/allocation, cash and Savings commit together; provenance-only envelope exclusion and Budget/Annual reconciliation preserve ordinary deposits. Whole reversal retains closed-history restrictions. SAV-06 now extends the same five installed account/event/guard/read functions in Migration034 for permanent nominal-month claims, explicit existing-cash fulfillment, skip/restoration and atomic due advancement. A separate protected Savings service/endpoint is disabled by default and processes one oldest occurrence per account; Loan scheduling is unchanged. No tables/views/signatures are added. SAV-07 adds the shared read-only reporting query in Migration035, consumed by Dashboard, Savings and Annual Summary, plus pre-pagination cash-flow classification/filtering in the two transaction readers. It uses one snapshot and exact decimal text to separate current holdings, effective-date ledger flows and transaction-date cash. No write function or relation is changed. The final inventory is two Savings tables, one summary view, eleven public Savings RPCs (one additional report reader) and eleven helper/trigger functions (ten private plus the read-only Budget exception). The owner accepted SAV-04–06 on 2026-09-14 and subsequently accepted SAV-07 and SAV-08; all implementation children are completed. Work remains local/uncommitted and undeployed. See the latest handoff for executed checks and isolated full-app preview.
...
All three application packages require Node.js `^20.19.0 || >=22.12.0` and carry the private package version `1.1.0` for the funded-budget minor release. Formal version tracking began with the v0.9.0 baseline; v1.0.0 remains the first stable release.
...
Migration 026 keeps that boundary unchanged. The combined current-month/recurring edit captures a read-only fingerprint, delegates the current funded half to the existing month-override command in the same PostgreSQL transaction, and then updates recurring configuration. Migration 027 extends that same command across existing future snapshots that still follow inherited/default planning: one current-month root owns deterministic child posting operations, while typed `month_override` items with `resolution_mode = recurring_propagation` distinguish inherited propagation from explicit override configuration. Explicit overrides and proven manual, copied, or late-resolution openings are skipped. `budget_movements` remains the only authority for every funded delta, all affected months commit or roll back together, and no table or view is added.

-- OWNER RUN ONLY; read-only, one consolidated JSON result. Requires Migration 034.
-- Pause application/import/job invocations across preflight -> 035 -> postflight.
-- Do not edit plan configuration for this reader upgrade. Compare evidence EXACTLY.
WITH history AS (
...
,
(SELECT md5(prosrc)='ebd503d3447f4b919f6057bfa25d6909' FROM pg_proc WHERE oid='public.savings_post_event_locked(uuid,jsonb)'::regprocedure) AS expected_event_engine,
(SELECT md5(prosrc)='466e1542ced0406484670f41dd80c4c9' FROM pg_proc WHERE oid='public.get_savings_account(bigint,date,date,bigint,integer)'::regprocedure) AS expected_history_reader
,
...
,
(SELECT count(*)=1 AND bool_and(md5(prosrc)='e7f340d80d22cd40a625696f1825bd4f' AND has_function_privilege('service_role',oid,'EXECUTE') AND NOT has_function_privilege('anon',oid,'EXECUTE') AND NOT has_function_privilege('authenticated',oid,'EXECUTE')) FROM pg_proc WHERE pronamespace='public'::regnamespace AND proname='transactions_filtered') AS expected_transactions_filtered,
(SELECT count(*)=1 AND bool_and(md5(prosrc)='ec7f14ccc8cd81fd88b58fc1141d96cc' AND has_function_privilege('service_role',oid,'EXECUTE') AND NOT has_function_privilege('anon',oid,'EXECUTE') AND NOT has_function_privilege('authenticated',oid,'EXECUTE')) FROM pg_proc WHERE pronamespace='public'::regnamespace AND proname='transactions_page') AS expected_transactions_page,
(SELECT count(*)=1 AND bool_and(md5(prosrc)='b104c3f64f50d5798a444bbf5f9ccb37' AND has_function_privilege('service_role',oid,'EXECUTE') AND NOT has_function_privilege('anon',oid,'EXECUTE') AND NOT has_function_privilege('authenticated',oid,'EXECUTE')) FROM pg_proc WHERE pronamespace='public'::regnamespace AND proname='get_savings_report') AS expected_get_savings_report
), evidence AS (SELECT jsonb_build_object(
 'unchanged_functions_fingerprint',(SELECT md5(string_agg(pg_get_functiondef(oid)||coalesce(proacl::text,''),E'\n' ORDER BY oid::regprocedure::text)) FROM pg_proc WHERE pronamespace='public'::regnamespace AND prokind='f' AND proname NOT IN ('transactions_filtered','transactions_page','get_savings_report')),
...
SELECT jsonb_build_object('result',CASE WHEN NOT EXISTS(SELECT 1 FROM jsonb_each(to_jsonb(c)) WHERE value<>'true'::jsonb) THEN 'MIGRATION_035_POSTFLIGHT_PASS' ELSE 'MIGRATION_035_POSTFLIGHT_FAIL' END,'checks',to_jsonb(c),'evidence',e.body) FROM checks c CROSS JOIN evidence e;

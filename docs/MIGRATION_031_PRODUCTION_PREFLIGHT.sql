-- OWNER RUN ONLY. Read-only; one consolidated JSON result. Requires Migration 030.
-- Pause application/import/scheduler writes across preflight -> 031 -> postflight.
...
to_regprocedure('public.post_savings_event(uuid,jsonb)') IS NULL AS migration_031_not_installed
...
SELECT jsonb_build_object('result',CASE WHEN NOT EXISTS(SELECT 1 FROM jsonb_each(to_jsonb(c)) WHERE value<>'true'::jsonb) THEN 'MIGRATION_031_PREFLIGHT_PASS' ELSE 'MIGRATION_031_PREFLIGHT_FAIL' END,'checks',to_jsonb(c),'evidence',e.body) FROM checks c CROSS JOIN evidence e;
